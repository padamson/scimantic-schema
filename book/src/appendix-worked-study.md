...
and letting the gaps it surfaces drive a polish pass in ch08. The
demand-driven dogfood made visible: this appendix is the demand, the
main body (especially ch08) is the supply, and the iteration between
...
PREREQUISITES (the study forces these; also tracked in ch08):
...
[ ] Identifiers — an id (identifier: true) slot so instances cross-
    reference into a DAG (ch08 deferral).
